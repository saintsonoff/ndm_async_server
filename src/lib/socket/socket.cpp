...
Socket::Socket(Socket&& other) noexcept : m_fd(other.m_fd) { other.m_fd = -1; }
...
TcpSocket::TcpCreateResult TcpSocket::create(int port) {
    ...
    if (bind(sock, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == -1) {
        return std::unexpected("Error bind TCP socket on port " +
                               std::to_string(port));
    }
...
std::expected<int, std::string> TcpSocket::accept_connection() {
    sockaddr_in client_addr{};
    socklen_t client_len = sizeof(client_addr);

    int client_fd = accept4(m_fd, reinterpret_cast<sockaddr*>(&client_addr),
    ...
    auto close_client = [](int* fd) {
        if (fd && *fd != Socket::kInvalidFd) {
            close(*fd);
        }
    };
    std::unique_ptr<int, decltype(close_client)> client_guard(&client_fd,
                                                              close_client);
...
UdpSocket::UdpCreateResult UdpSocket::create(int port) {
    ...
    if (bind(sock, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == -1) {
        return std::unexpected("Error bind UDP socket on port " +
                               std::to_string(port));
    }
